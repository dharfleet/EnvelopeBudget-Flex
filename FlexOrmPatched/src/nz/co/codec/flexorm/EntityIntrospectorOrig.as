package nz.co.codec.flexorm
{
	...
	public class EntityIntrospectorOrig
	{
		...
		public function EntityIntrospectorOrig(schema : String, sqlConnection : SQLConnection, entityMap : Object, debugLevel : int, prefs : Object=null)
		{
			_schema = schema;
		...
		private function loadMetadataForClass(cls : Class) : Entity
		{
			...
			for each (var v : Object in variables)
			{
				...
				{
					column = usingCamelCaseNames() ? property : StringUtils.underscore(property).toLowerCase();
					...
					{
						defaultKey = new PrimaryKey({column: column, property: property, strategy: getIDStrategy(v.@type), type: getSQLType(v.@type)});
					}
				}
				...
				if (missingKey && v.metadata.(@name == Tags.ELEM_ID).length() > 0)
				{
					...
					entity.addKey(new PrimaryKey({column: column, property: property, strategy: strategy, type: getSQLType(v.@type)}));
					missingKey = false;
				}
			}
		...
		private function extractManyToOneAssociation(v : Object, entity : Entity, property : String) : void
		{
			...
			entity.addManyToOneAssociation(new Association({property: property, fkColumn: fkColumn, fkProperty: fkProperty, associatedEntity: associatedEntity, cascadeType: cascadeType, inverse: inverse, constrain: constrain, hierarchical: associationIsHierarchical}));
		...
		private function extractInferredManyToOneAssociation(entity : Entity, type : Class, property : String) : void
		{
			...
			entity.addManyToOneAssociation(new Association({property: property, fkColumn: associatedEntity.fkColumn, fkProperty: associatedEntity.fkProperty, associatedEntity: associatedEntity, hierarchical: associationIsHierarchical}));
			entity.addDependency(associatedEntity);
		}
		...
		private function extractOneToManyAssociation(v : Object, entity : Entity, property : String, c_n : String) : void
		{
			...
			var a : OneToManyAssociation = new OneToManyAssociation({property: property, multiTyped: (types.length > 1), associatedTypes: associatedTypes, cascadeType: cascadeType, lazy: lazy, inverse: inverse, constrain: constrain, fkColumn: fkColumn, fkProperty: fkProperty, indexed: indexed, indexColumn: indexColumn, indexProperty: indexProperty});
		...
		private function extractInferredOneToManyAssociation(entity : Entity, type : Class, property : String) : void
		{
			...
			var a : OneToManyAssociation = new OneToManyAssociation({property: property, associatedEntity: associatedEntity, fkColumn: entity.fkColumn, fkProperty: entity.fkProperty});
			associatedEntity.addOneToManyInverseAssociation(a);
			entity.addOneToManyAssociation(a); // also sets the ownerEntity as entity
			associatedEntity.addDependency(entity);
		}
		...
		private function extractManyToManyAssociation(v : Object, entity : Entity, property : String, c_n : String) : void
		{
			...
			var a : ManyToManyAssociation = new ManyToManyAssociation({property: property, associationTable: associationTable, associatedEntity: associatedEntity, cascadeType: cascadeType, lazy: lazy, constrain: constrain, indexed: indexed, indexColumn: indexColumn, indexProperty: indexProperty});
		...
		private function getIdentities(entity : Entity, path : Array=null) : Array
		{
			...
			for each (var key : Key in entity.keys)
			{
				...
				else
				{
					...
					if (path.length == 0)
					{
						identities.push(new Identity({property: pk.property, column: pk.column, fkProperty: entity.fkProperty, fkColumn: entity.fkColumn, strategy: pk.strategy, path: []}));
					}
					else
					{
						identities.push(new Identity({property: entity.fkProperty, column: entity.fkColumn, fkProperty: entity.fkProperty, fkColumn: entity.fkColumn, strategy: pk.strategy, path: path.concat(key)}));
					}
				}
			}
			return identities;
...
//                idSQLType = getSQLTypeForID(pk.strategy);
		...
		private function loadMetadataForObject(obj : Object, name : String, root : String) : Entity
		{
			...
			{
				var column : String = usingCamelCaseNames() ? property : StringUtils.underscore(property);
				...
				if (value)
				{
					...
					if ("Object" == cn)
					{
						...
						entity.addManyToOneAssociation(new Association({property: property, associatedEntity: associatedEntity, fkColumn: associatedEntity.fkColumn, fkProperty: associatedEntity.fkProperty}));
					}
					...
					else if ((value is Array || value is ArrayCollection) && (value.length > 0))
					{
						...
						var a : OneToManyAssociation = new OneToManyAssociation({property: property, associatedTypes: associatedTypes, fkColumn: entity.fkColumn, fkProperty: entity.fkProperty});
						associatedEntity.addOneToManyInverseAssociation(a);
						entity.addOneToManyAssociation(a); // also sets the ownerEntity as entity
					}
					...
					{
						entity.addField(new Field({property: property, column: column, type: getSQLType(cn)}));
					}
				}
			}
			...
			var key : String = "__id";
			obj[key] = 0;
			entity.addKey(new PrimaryKey({property: key, column: entity.fkColumn, strategy: IDStrategy.AUTO_INCREMENT}));
			entity.identities = getIdentities(entity);
			buildSQLCommands(entity);
			entity.createSynCommand.execute();
			entity.initialisationComplete = true;
